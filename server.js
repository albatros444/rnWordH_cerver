...
app.post("/", (req, res) => {
  ...
  if (req.body.type === "sentences") {
    content = `Give me 5 simple sentences using the word ${req.body.wordForSentences}. Give no explanations. You must return them in next format: 1. Some sentence here. &00& 2. Some other sentence here. &00& etc.`;
  ...
  try {
    client
      ...
      .then((resp) => {
        console.log(
          "got response from gigachat",
          resp.choices[0]?.message.content
        );
        if (req.body.type === "sentences") {
          const splSentences = resp.choices[0]?.message.content.split(/&00&/);
